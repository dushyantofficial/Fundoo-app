...
//use App\Models\Admin\Valet_InquiryDatatable;
use App\Models\Admin\PermanentInquiry;
use Yajra\DataTables\EloquentDataTable;
use Yajra\DataTables\Services\DataTable;

class Valet_InquiryDatatable extends DataTable
{
    /**
...
//            ->addColumn('action', 'admin.permanent_driver.datatables_actions')
            ...
            ->editColumn('mobile_no', function ($record) {
                return $record->valet_customer->mobile_no;
...
//                return $record->created_at->format('d-m-Y h:i:s');
            ...
            ->editColumn('name', function ($record) {
                return $record->valet_customer->first_name . ' ' . $record->valet_customer->middle_name . ' ' . $record->valet_customer->last_name;
            })
...
//                dd($record->user->status);
     ...
     *
     * @param \App\Models\Admin\Valet_InquiryDatatable $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(PermanentInquiry $model)
    {
        $model = PermanentInquiry::where('status', config('constants.STATUS.Pending'))->where('type', config('constants.CATEGORY.valet_parking'));
        return $model->newQuery()->with('valet_customer');
    }
...
//            ->addAction(['width' => '120px', 'printable' => false])
    ...
    protected function getColumns()
    {
        return [
            ...
            'name' => ['data' => 'name', 'name' => 'valet_customer.first_name', 'title' => 'Customer Name', 'searchable' => true],
            'created_at',
            'mobile_no' => ['data' => 'valet_customer.mobile_no', 'name' => 'valet_customer.mobile_no', 'title' => 'Mobile No'],
            'work_type',
            'no_of_drivers',
            'status',
            'actions'
    ...
    {
        return 'Admin\Valet_Inquiry_' . date('YmdHis');
    }
}
