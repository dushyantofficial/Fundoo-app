...
use App\DataTables\Admin\PickUp_Drop_DriverDataTable;
...
class PickUp_Drop_DriverController extends Controller
{
    public function index(PickUp_Drop_DriverDataTable $PickUp_Drop_DriverDataTable)
    {

        return $PickUp_Drop_DriverDataTable->render('admin.pickUp_drop_driver.index');
    }
    ...
    public function create()
    {
        ...
        return view('admin.pickUp_drop_driver.create', compact('category_name', 'work_categorys','user', 'work', 'states', 'city'));
    }
...
//        ]);
        $input = $request->all();
        $input['work_type'] = config('constants.CATEGORY.pickup-drop');
...
//        $input['mobile_no'] = $request->aditional_contact_no;
        ...
        return redirect(route('admin.pickUp-drop-driver.index'))->with('success', 'PickUp Drop Driver saves successfully...');

    }

    /**
     * Display the specified pickUp_drop_driver.
     *
     * @param int $id
     *
     * @return Response
    ...
    public function show($id)
    {
        ...
        if (empty($permanent_driver)) {
            return redirect(route('admin.pickUp-drop-driver.index'))->with('danger', 'Driver not found');
        }

        return view('admin.pickUp_drop_driver.show')->with('permanent_driver', $permanent_driver);
    }

    /**
     * Show the form for editing the specified pickUp_drop_driver.
     *
     * @param int $id
     *
     * @return Response
    ...
    public function edit($id)
    {
        ...
        return view('admin.pickUp_drop_driver.edit', compact('category_name', 'work_categorys','userss','work', 'array', 'driverExtended', 'states', 'city'));
    }

    /**
     * Update the specified pickUp_drop_driver in storage.
     *
     * @param int $id
...
//            'aditional_contact_no' => 'required|unique:driver_extendeds,aditional_contact_no,' . $driverExtended->id
        ]);
        $input = $request->all();
...
//        $users['mobile_no'] = $request->aditional_contact_no;
        ...
        if (empty($driverExtended)) {
            ...
            return redirect(route('admin.pickUp-drop-driver.index'));
        }
        $user->update($users);
        $driverExtended->update($input);


        return redirect(route('admin.pickUp-drop-driver.index'))->with('info', 'PickUp Drop Driver updated successfully...');
    }

    /**
     * Remove the specified pickUp_drop_driver from storage.
     *
     * @param int $id
     *
     * @return Response
    ...
    public function destroy($user_id)
    {
        ...
        return redirect(route('admin.pickUp-drop-driver.index'))->with('danger', 'PickUp Drop Driver deleted successfully...');
    }
