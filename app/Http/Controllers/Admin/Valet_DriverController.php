...
use App\DataTables\Admin\Valet_DriverDataTable;
...
class Valet_DriverController extends Controller
{
    public function index(Valet_DriverDataTable $Valet_DriverDataTable)
    {

        return $Valet_DriverDataTable->render('admin.valet_driver.index');
    }
    ...
    public function create()
    {
        ...
        return view('admin.valet_driver.create', compact('category_name', 'work_categorys','user', 'work', 'states', 'city'));
    }
...
//        ]);
        $input = $request->all();
        $input['work_type'] = config('constants.CATEGORY.valet_parking');
...
//        $input['mobile_no'] = $request->aditional_contact_no;
        ...
        return redirect(route('admin.valet-driver.index'))->with('success', 'Valet Driver saves successfully...');
    ...
    public function show($id)
    {
        ...
        $valet_driver = $this->driverExtendedRepository->find($id);

        if (empty($valet_driver)) {
            return redirect(route('admin.permanent-driver.index'))->with('danger', 'Driver not found');
        }

        return view('admin.valet_driver.show')->with('valet_driver', $valet_driver);
    }

    /**
     * Show the form for editing the specified valet-driver.
     *
     * @param int $id
     *
     * @return Response
    ...
    public function edit($id)
    {
        ...
        if (empty($driverExtended)) {
            ...
            return redirect(route('admin.valet-driver.index'))->with('danger', 'Driver not found');
        }
        $work_categorys = DriverCategory::pluck('category_name','id');
        $category_name = DriverCategory::all();
        $states = State::pluck('state_name','id');
        ...
        return view('admin.valet_driver.edit', compact('category_name', 'work_categorys','userss','work', 'array', 'driverExtended', 'states', 'city'));
    }

    /**
     * Update the specified valet-driver in storage.
     *
     * @param int $id
...
//        $users['mobile_no'] = $request->aditional_contact_no;
        ...
        if (empty($driverExtended)) {
            ...
            return redirect(route('admin.valet-driver.index'));
        }
        $user->update($users);
        $driverExtended->update($input);


        return redirect(route('admin.valet-driver.index'))->with('info', 'Valet Driver updated successfully...');
    }
    ...
    public function destroy($user_id)
    {
        ...
        $valet_driver = User::find($user_id);
        $valet_driver->delete();

        Flash::success('Driver Extended deleted successfully.');

        return redirect(route('admin.valet-driver.index'))->with('danger', 'Valet Driver deleted successfully...');
    }
