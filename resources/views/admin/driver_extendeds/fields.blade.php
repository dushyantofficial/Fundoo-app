...
<div class="form-group col-sm-2">
    ...
    @if(isset($driverExtended))
        <select name="work_type" class="form-control select2" id="work_type_edit" onchange="changeeditWork()">
            <option>Please Select Work Category</option>
            @foreach(config('constants.CATEGORY') as $value => $values)
                <option
                    value="{{ $value }}" {{ ($driverExtended->work_type==$value)? "selected" : "" }}>
                    {{ $values }}
                </option>
            @endforeach
        </select>
    @else
        <select name="work_type" class="form-control select2" id="work_type" onchange="changeWork()">
            <option>Please Select Work Category</option>
            @foreach(config('constants.CATEGORY') as $value => $values)
                <option
                    value="{{ $value }}">
    ...
    @endif
    <span style="color: red">{{$errors->first('work_type')}}</span>
</div>

<div class="form-group col-sm-3">

    {!! Form::label('status', 'Multi Select Work Type') !!}

    <select class="form-control select2" multiple="multiple" data-live-search="true"
            name="multi_work_type[]" placeholder="Multi Select Work Type" style="width: 100%; ">
        @foreach(config('constants.CATEGORY') as $value => $values)
            @if(!empty($driverExtended->multi_work_type))
                <option value="{{ $value }}"
                        @if(isset($driverExtended)) @if(in_array($value,$driverExtended->multi_work_type)) selected @endif @endif>
                    {{ $values }}
                </option>
            @else
    ...
    <span class="text-danger">{{$errors->first('multi_work_type')}}</span>
</div>
...
{{--    {!! Form::text('mobile_no', null, ['class' => 'form-control'.($errors->has('mobile_no') ? 'is-invalid': ''),"Placeholder" => 'Enter Mobile No','id' => 'mobile_no']) !!}--}}
...
<div class="form-group col-sm-2">
    ...
    @if(isset($userss))
        <input type="date" name="date_of_birth" class="form-control @error('date_of_birth') is-invalid @enderror" value="{{formate_date($userss->date_of_birth)}}">
    @else
        <input type="date" name="date_of_birth" class="form-control @error('date_of_birth') is-invalid @enderror" value="{{old('date_of_birth')}}" palceholder="Enter Date_Of_Birth">
    @endif
...
@if(isset($driverExtended))
    <div class="form-group col-sm-2" id="editmonthlysalary">
        {!! Form::label('monthly_salary', 'Monthly Salary:') !!}
        {!! Form::number('monthly_salary', null, ['class' => 'editsalary form-control '.($errors->has('monthly_salary') ? 'is-invalid': ''),'Placeholder'=>'Enter Monthly Salary']) !!}
        @error('monthly_salary')
        <span class="error invalid-feedback">{{ $message }}</span>
        @enderror
    </div>
@else
    <div class="form-group col-sm-2" id="monthlysalary">
        {!! Form::label('monthly_salary', 'Monthly Salary:') !!}
        {!! Form::number('monthly_salary', null, ['class' => 'theInput form-control '.($errors->has('monthly_salary') ? 'is-invalid': ''),'Placeholder'=>'Enter Monthly Salary']) !!}
        @error('monthly_salary')
        <span class="error invalid-feedback">{{ $message }}</span>
        @enderror
    </div>
...
<div class="container-fluid">
    <div class="row pb-4 pt-4">
        <div class="col-lg-6 border-right">
            ...
            <div class="row pt-4">
                ...
                <div class="form-group col-sm-2 d-none">
                    ...
                    {!! Form::text('pincode', null, ['class' => 'form-control '.($errors->has('pincode') ? 'is-invalid': ''),"Placeholder" => 'Enter Pincode','min' => '6','max' => '6']) !!}
                    @error('pincode')
                    <span class="error invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
            ...
            {{--                    <br>--}}
            {{--                    <div class="form-check form-check-inline">--}}
            {{--                        <input class="form-check-input" type="radio" data-id="address_type" name="resi_ads_type"--}}
            {{--                               value="rhome"--}}
            {{--                               {{old('resi_ads_type')=="rhome" ? 'checked='.'"checked"' : '' }}  checked>--}}
            {{--                        <label class="form-check-label">Home</label>--}}
            {{--                    </div>--}}
            {{--                    <div class="form-check form-check-inline">--}}
            {{--                        <input class="form-check-input" type="radio" name="resi_ads_type" data-id="office"--}}
            {{--                               value="roffice" {{old('resi_ads_type')=="roffice" ? 'checked='.'"checked"' : '' }}>--}}
                ...
                <div class="col-lg-7">
                    ...
                    <div class="form-group">
                        ...
                        @if(isset($driverExtended))
                            ...
                            @else
                                <img src="{{$driverExtended->resi_ads_proof}}"
        ...
        <div class="col-lg-6">
            ...
            {{--                    <br>--}}
            {{--                    <div class="form-check form-check-inline">--}}
            {{--                        <input class="form-check-input" type="radio" name="post_ads_type" value="home" id="address_type"--}}
            {{--                               {{old('post_ads_type')=="home" ? 'checked='.'"checked"' : '' }}  checked>--}}
            {{--                        <label class="form-check-label">Home</label>--}}
            {{--                    </div>--}}
            {{--                    <div class="form-check form-check-inline">--}}
            {{--                        <input class="form-check-input" type="radio" name="post_ads_type" id="office"--}}
            {{--                               value="office" {{old('post_ads_type')=="office" ? 'checked='.'"checked"' : '' }}>--}}
            {{--                        <label class="form-check-label">Office</label>--}}
            {{--                    </div>--}}
            {{--                    @error('gander')--}}
            {{--                    <span class="error invalid-feedback">{{ $message }}</span>--}}
                        ...
                        @if(isset($driverExtended))
                            @if($driverExtended->post_ads_proof == '')
                                <img src="{{asset('admin/dummy_image/no-image.jpg')}}" alt="user"
                                     style="width: 40px; height: 40px;">
                            @else
                                <img src="{{$driverExtended->post_ads_proof}}"
                                     style="width: 40px; height: 40px;"
                                     alt="user">
                            @endif
                        @endif
...
<div class="form-group col-sm-2">
    {!! Form::label('work_station', 'Work Out Of Station:') !!}
    <select name="work_station" class="form-control select2">
        @foreach(config('constants.WORK_STATION') as $value => $values)
            <option
                value="{{ $value }}" @if(isset($driverExtended)){{ ($driverExtended->work_station==$value)? "selected" : "" }} @endif>
                {{ $values }}
            </option>
        @endforeach
    </select>
...
<div class="form-group col-sm-2">
    ...
    <select name="work_time" class="form-control select2">
        @foreach(config('constants.WORK_TIME') as $value => $values)
            ...
            <option
                value="{{ $value }}" @if(isset($driverExtended)){{ ($driverExtended->work_time==$value)? "selected" : "" }} @endif>
                {{ $values }}
            </option>
        @endforeach
    </select>
...
<div class="form-group col-sm-6">
    ...
    {!! Form::label('status', 'Select Language:') !!}
    ...
    <select class="form-control select2" data-live-search="true"
        ...
        @foreach(config('constants.LANGUAGE_KHOWN') as $value => $values)
            <option
                value="{{ $value }}" @if(isset($driverExtended)){{ ($driverExtended->language_known==$value)? "selected" : "" }} @endif>
                {{ $values }}
            </option>
        @endforeach
    </select>
    <span class="text-danger">{{$errors->first('language_known')}}</span>
</div>
    ...
    <div class="form-group">
        ...
        @if(isset($driverExtended))
            @if($driverExtended->license == '')
                <img src="{{asset('admin/dummy_image/no-image.jpg')}}" alt="user" style="width: 40px; height: 40px;">
            @else
                <img src="{{$driverExtended->license}}"
                     style="width: 40px; height: 40px;"
                     alt="user">
            @endif
        @endif
    ...
    <div class="form-group">
        ...
        @if(isset($driverExtended))
            @if($driverExtended->aadhar_card == '')
                <img src="{{asset('admin/dummy_image/no-image.jpg')}}" alt="user" style="width: 40px; height: 40px;">
            @else
                <img src="{{$driverExtended->aadhar_card}}"
                     style="width: 40px; height: 40px;"
                     alt="user">
            @endif
        @endif
    ...
    <div class="form-group">
        ...
        @if(isset($driverExtended))
            @if($driverExtended->pancard == '')
                <img src="{{asset('admin/dummy_image/no-image.jpg')}}" alt="user" style="width: 40px; height: 40px;">
            @else
                <img src="{{$driverExtended->pancard}}"
                     style="width: 40px; height: 40px;"
                     alt="user">
            @endif
        @endif
    ...
    <div class="form-group">
        ...
        @if(isset($driverExtended))
            @if($driverExtended->light_bill == '')
                <img src="{{asset('admin/dummy_image/no-image.jpg')}}" alt="user" style="width: 40px; height: 40px;">
            @else
                <img src="{{$driverExtended->light_bill}}"
                     style="width: 40px; height: 40px;"
                     alt="user">
            @endif
        @endif
...
<div class="form-group col-sm-6">
    ...
    <select name="bank_name" class="form-control select2">
        ...
        @foreach(config('constants.INDIA_BANKS') as $value => $values)

            <option
                value="{{ $value }}" @if(isset($driverExtended)){{ ($driverExtended->bank_name==$value)? "selected" : "" }} @endif>
                {{ $values }}
            </option>
        @endforeach
    </select>
    ...
    <!-- Branch Name Field -->
    <div class="form-group col-sm-6">
        {!! Form::label('branch_name', 'IFSC Code:') !!}
        {!! Form::text('branch_name', null, ['class' => 'form-control '.($errors->has('branch_name') ? 'is-invalid': ''),'Placeholder'=>'Enter IFSC Code','id' => 'ifsc_code']) !!}
        @error('branch_name')
        <span class="error invalid-feedback">{{ $message }}</span>
        @enderror
    </div>
    @push('page_scripts')
        <script>
            $(document).ready(function () {
                var selected_type = $('#work_type_edit').val();
                var permanent = '<?php echo config('constants.CATEGORY.permanent'); ?>';
                var temporary = '<?php echo config('constants.CATEGORY.temporary'); ?>';
                var valet_parking = '<?php echo config('constants.CATEGORY.valet_parking'); ?>';
                var pickup_drop = '<?php echo config('constants.CATEGORY.pickup-drop'); ?>';

                if (selected_type == permanent) {
                    $('#editmonthlysalary').show();

                } else if (selected_type == temporary) {
                    $('#editmonthlysalary').hide();

                } else if (selected_type == valet_parking) {
                    $('#editmonthlysalary').show();

                } else if (selected_type == pickup_drop) {
                    $('#editmonthlysalary').hide();
                }
            });
        </script>
        <script>
            function changeeditWork() {
                var selected_type = $('#work_type_edit').val();
                var permanent = '<?php echo config('constants.CATEGORY.permanent'); ?>';
                var temporary = '<?php echo config('constants.CATEGORY.temporary'); ?>';
                var valet_parking = '<?php echo config('constants.CATEGORY.valet_parking'); ?>';
        ...
        <script>
            function changeWork() {
                var selected_type = $('#work_type').val();
                var permanent = '<?php echo config('constants.CATEGORY.permanent'); ?>';
                var temporary = '<?php echo config('constants.CATEGORY.temporary'); ?>';
                var valet_parking = '<?php echo config('constants.CATEGORY.valet_parking'); ?>';
                var pickup_drop = '<?php echo config('constants.CATEGORY.pickup-drop'); ?>';
        ...
        {{--            var pincode = jQuery('#myInput3').val();--}}
        {{--            if (pincode == '') {--}}
        {{--                jQuery('#city').val('');--}}
        {{--                jQuery('#state').val('');--}}
        {{--            } else {--}}
