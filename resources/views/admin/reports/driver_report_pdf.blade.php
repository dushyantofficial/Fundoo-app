                ...
                <div id="leaderBoardSwissTable" class="print-table">
                    <table id="leader" class="table table-responsive" border="1" width="100%"
                        ...
                        <tr class="text-center">
                            ...
                            <th style="padding: 10px;">Aditional No</th>
                            <th style="padding: 10px;">Email</th>
                            <th style="padding: 10px;">Work Type</th>
                        ...
                        @foreach($drivers as $driver)
                            <tr style="text-align: center;">
                                ...
                                @if($driver->user_id)
                                    <td style="padding: 10px;">{{ $driver->user->first_name }} {{ $driver->user->middle_name }} {{ $driver->user->last_name }}</td>
                                @else
                                    <td>-</td>
                                @endif
                                <td style="padding: 10px;">{{ $driver->user->mobile_no }}</td>
                                <td style="padding: 10px;">{{ $driver->aditional_contact_no }}</td>
                                <td style="padding: 10px;">{{ $driver->user->email }}</td>
                                <td style="padding: 10px;">{{ $driver->work_type }}</td>
                                <td style="padding: 10px;">{{ $driver->user->city }}</td>
                                ...
                                <td style="padding: 10px;">{{ $driver->created_at }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
