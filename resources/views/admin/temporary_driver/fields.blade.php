...
{{--    {!! Form::select('work_type', $work_categorys,old('work_type'), ['class'=>'form-control select2 '.($errors->has('work_type') ? 'is-invalid': ''),'placeholder'=>'Please Select Work Category','id'=>'work_type']); !!}--}}
    ...
    <input type="text" name="work_type" id="work_type" class="form-control"
           value="{{config('constants.CATEGORY.temporary')}}"
           @if(isset($driverExtended)){{ ($driverExtended->work_type==config('constants.CATEGORY.temporary'))? "selected" : "" }} @endif disabled>
...
@push('page_scripts')
    ...
    {{--            var temporary = '<?php echo config('constants.CATEGORY.temporary'); ?>';--}}
    {{--            if (selected_type == temporary) {--}}
    {{--                $('#monthlysalary').attr('disabled', 'diasbled');--}}
